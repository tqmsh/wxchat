...
export function useConversations(userId = "user123") {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        const sortedConversations = conversationUtils.sortConversationsByDate(
          result.data
        );
  ...
  const updateConversation = useCallback(async (conversationId, updateData) => {
    try {
      ...
      const result = await conversationAPI.updateConversation(
        conversationId,
        updateData
      );
  ...
  const filteredConversations = conversationUtils.filterConversations(
    conversations,
    searchTerm
  );
