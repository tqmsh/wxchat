...
def process_subimages_from_zip(zip_path: str) -> str:
    """
    ...
    """
    prompt_text = "The image contains a snippet from a report that may have text, tables, or images in it. Convert all text in the image into valid markdown. It is critical that you convert the text faithfully. Do not make anything up. Make sure you convert all text. You must respect the formatting, especially for tabular data and headings."
...
def ocr_pdf(pdf_path, project_name=None, results_dir="ocr", save_to_file=False, cleanup_zip=True):
    """
    ...
    Args:
        ...
        cleanup_zip (bool, optional): Whether to delete the zip files after processing. Defaults to True.

    Returns:
        str: The extracted text from the PDF file.
    """

    # Set up the project name and directories with a unique identifier
    if project_name is None:
        project_name = os.path.splitext(os.path.basename(pdf_path))[0]
    
    # Add a unique ID to ensure no duplicate directories
    unique_id = time.strftime("%Y%m%d-%H%M%S") + "-" + str(uuid.uuid4().hex[:6])
    project_name = f"{project_name}_{unique_id}"
    ...
    for root, dirs, filenames in os.walk(output_directory):
        ...
        for filename in filenames:
            if filename.endswith('.jpg'):
                ...
                if zip_path:
                    running_text += "\n" + process_subimages_from_zip(zip_path).replace("```markdown", "").replace("```", "")
                    if cleanup_zip:
                        os.remove(zip_path)  # Clean up the zip file if requested
...
#Turns a PDF file into a markdown file then saves it.    
def pdf_to_md(pdf_path,output_directory = "."):
    md_text = pymupdf4llm.to_markdown(pdf_path)
    # Extract the filename from pdf_path
    file_name = os.path.basename(pdf_path)
    
    # Remove the .pdf extension and replace it with .txt
    text_file_name = file_name.replace(".pdf", ".txt")
    pathlib.Path(os.path.join(output_directory,text_file_name)).write_bytes(md_text.encode())