...
async def process_form(
    ...
    download_link = f"/download/{project_name}/{zip_filename}"
    return templates.TemplateResponse("index.html", {"request": request, "download_link": download_link})
...
async def download_file(project_name: str, file_name: str):
    file_location = os.path.join("temp", project_name, file_name)
    if os.path.exists(file_location):
        return FileResponse(path=file_location, media_type='application/zip', filename=file_name)
    else:
